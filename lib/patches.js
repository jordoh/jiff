...
function invertAdd(pr, add) {
	...
	if(context !== void 0) {
		context = clone(add.context);
		context.before = add.context.before;
		context.after = array.cons(add.value, add.context.after);
	}
...
function invertReplace(pr, c, i, patch) {
	...
	if(context !== void 0) {
		...
		context.before = prev.context.before;
		context.after = array.cons(prev.value, array.tail(prev.context.after));
	}
...
function invertRemove(pr, c, i, patch) {
	...
	if(context !== void 0) {
		context = clone(prev.context);
		context.before = prev.context.before;
		context.after = array.tail(prev.context.after);
	}
